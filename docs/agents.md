...
1. Crée la politique XDG AgentRun avec les **38 profils** du fichier initial, uniquement si aucune configuration n'existe. La racine initiale est `~/.codex/worktrees`.
...
3. Ajoute les consignes permanentes au fichier global `AGENTS.md` dans `CODEX_HOME` (défaut `~/.codex`). Un `AGENTS.override.md` non vide est utilisé en priorité. Le bloc délimité par `<!-- agentrun:instructions:start -->` / `<!-- agentrun:instructions:end -->` est géré par l'utilitaire ; le reste du fichier est conservé.
...
Depuis 0.4.0, utiliser les profils Make pour les serveurs au premier plan et les outils de stack pour Docker Compose local. Un `make up` qui délègue à Docker doit être examiné pour déclarer la recette Compose existante, sans ajouter de wrapper ni réécrire le projet. systemd et les fonctionnalités Compose non prises en charge conservent leur workflow existant. Voir le [guide Compose et Makefiles](service-managers.md).
...
Le fichier initial contient **38 profils** ; voir le [catalogue complet et ses prérequis](profiles.md). La copie ci-dessus les installe tous pour une nouvelle configuration. Voici un extrait minimal :
...
La réponse doit provenir de l'outil MCP et contenir `version: 1` et une liste `processes`, éventuellement vide. Vérifier ensuite les outils de processus : `list_processes`, `get_process`, `start_process`, `stop_process`, `restart_process`, `get_logs`, `clean_registry`. Six outils de stack sont également disponibles : `list_stacks`, `get_stack`, `start_stack`, `stop_stack`, `restart_stack`, `get_stack_logs`.
