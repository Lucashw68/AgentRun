...
<!-- agentrun:instructions:end -->'''
        ...
        self.assertIn('Do not call AgentRun for ordinary edits', upgraded)
        self.assertIn('do not list again merely\n  to finish', upgraded)
        self.assertNotIn('before completing a task', upgraded)
        self.assertIn('Before start_process, restart_process, start_stack or restart_stack', upgraded)
        self.assertIn('Report all known blockers together', upgraded)
        self.assertIn('If the client blocks a call, AgentRun did not execute', upgraded)
        self.assertIn('Do not alter project files just to use', upgraded)
        self.assertIn('do not treat an unsupported workload as a missing profile', upgraded)
        documented = (bundle / 'docs/agents.md').read_text().split('## 4. Donner une consigne permanente à l\'agent', 1)[1].split('```md\n', 1)[1].split('\n```', 1)[0]
        self.assertIn('<!-- agentrun:instructions:start -->\n' + documented + '\n<!-- agentrun:instructions:end -->', upgraded)
